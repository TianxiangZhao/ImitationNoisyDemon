...
from plot.plot_tsne import plot_tsne,plot_pca
...
def tensor_distance_euc(tensor1, tensor2):
    distance = [F.pairwise_distance(emb, tensor2) for emb in tensor1]
    distance = torch.stack(distance)
...
class SkillBC(object):
    def __init__(self, observation_space, action_space, demonstrations,
                 ...
                 test_demonstrations=None, logger=None, env=None, valid_demon=None):
        """
        ...
        self.feature_ext = models.MLPFeature(in_dim=alg_args['interval']*np.prod(observation_space.shape),
                                                         hid_dim=alg_args['hid_dim'], out_dim=alg_args['hid_dim'],
                                                         dropout=alg_args['dropout']).to(self.device)
        self.future_feature_ext = models.MLPFeature(in_dim=alg_args['future_interval'] * np.prod(observation_space.shape),
                                                         hid_dim=alg_args['hid_dim'], out_dim=alg_args['hid_dim'],
                                                         dropout=alg_args['dropout']).to(self.device)
        ...
        self.match_net = models.MatchNet(proto_dim=alg_args['proto_dim'], proto_num=alg_args['proto_num'],
                                         hard_sel=alg_args['hard_sel'],
                                         tensor_dis_func=self.distance_func).to(self.device)

        self.skill_feature_ext = models.MLPFeature(in_dim=np.prod(observation_space.shape),
                                                         hid_dim=alg_args['hid_dim'], out_dim=alg_args['hid_dim'],
                                                         dropout=alg_args['dropout']).to(self.device)
    ...
    def compute_mi(self, states, S_em, actions, cluster_label =None, DEC=0.0, random_prob=0.2):
        """
            ...
            if True: # sampling negative pairs
                if cluster_label is not None and random.random()>DEC:
                    ...
                    if cluster_label is not None:
                        op_label = (cluster_label<=3).float()
    ...
    def train_batch(self, demon_batch, use_bi=False, opt_all=True, weight=None):
        ...
        # calculate loss
        # -------------
        if use_bi is False and self.pre_trained: # to adapt the high-level policy with knowledge distill
            with torch.no_grad():
        ...
        if not self.pre_trained:
            ...
            #condE_loss = self.compute_mi(states=states, S_em=info['pre_skill_emb'],actions=actions,
            #                             cluster_label=clust_label, DEC = self.DEC)
            ...
            losses = loss_bc + self.args['lambda_mi']*(condE_loss-S_entropy_batch) + \
    ...
    def train_pseudo(self, demon_batch, use_bi=False,):
        ...
        if use_bi is False and self.pre_trained:
            losses += skill_distill_loss
    ...
    def test_batch(self, demon_batch, use_bi=False, return_info = False, deterministic=False):
        ...
        #
        '''
        print("test bc loss: {}, condE loss: {}, S_entropy_batch: {}, proto_div_loss: {}, S_entropy_ins: {}".format(
            loss_bc.item(), condE_loss.item(), S_entropy_batch.item(), proto_div_loss.item(), S_entropy_ins.item()
        ))
        '''
    ...
    def train_all(self, epochs=10, use_bi=False, eval_env = None, prefix='all', opt_all=True, weights=None):
        """
    ...
    def propagate_score(self, expert_batches, unknown_batches, threshold=0.2, use_bi=True):
