...
load_goals = ['_opt','_noisy'] #[13, 12]
# load_goals = [[13, 16]]
load_test_goals = ['_opt','_noisy' ]
train_epoch = 70
left_interval = 5
right_interval = 1
batch_size = 32
...
for lambda_mi in lambda_mis:
    ...
    tensorboard_log = 'logs_{}/_{}_new/load{}train{}_ratio{}_PUinter{}/lr{}_minew{}_clus{}PU{}thresh{}DEC{}bi{}sm{}_pseudo{}pre{}_hardsel{}temper{}_bs{}negK{}_loadbest{}'.format(env_name, method, load_goals,
    ...
    batched_demon = exp.load_batched_demon(load_goals=load_goals, traj_dir=traj_dir, batch_size=batch_size,
                                           ...
                                           pos_ratio=train_ratio,label='train')
    test_batched_demons = []
    ...
    if method == 'bi-modeling':
        ...
        trainer = skill_bc.SkillBC(observation_space=env.observation_space, action_space=env.action_space,
                                   ...
                                   env=env, correct_softmax=correct_softmax,extra_dim=2)
        learner = trainer
    ...
    save_path = 'logs_{}/pretrained_{}'.format(env_name,checkpoint_name)
